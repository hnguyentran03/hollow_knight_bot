...
def test_command_never_forwards_timescale(tmp_path, stub_trainer):
    # Timescale training is disabled: the launcher drops the param
    # entirely in every mode, so train.py always runs at 1x.
    for params in ({"run_id": "r1"},
                   {"run_id": "r1", "timescale": 2},
                   {"mode": "resume", "run_id": "old", "timescale": 2},
                   {"run_id": "r1", "timescale": ""}):
        assert "--timescale" not in launcher.command(tmp_path, dict(params),
                                                     platform="linux")


def test_restart_params_drops_recorded_timescale(tmp_path):
    # A run recorded at 2x (pre-disable) restarts without the param;
    # train.py forces 1x either way.
    run = tmp_path / "runs" / "r1"
    run.mkdir(parents=True)
