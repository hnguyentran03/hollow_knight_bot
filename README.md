...
Useful flags: `--boss` (default `hornet1`; ids from `hkrl/bosses.py`), `--instances N`, `--gen-every` (checkpoint interval, default 15000), `--n-steps` / `--batch-size` / `--n-epochs` (fresh runs only), `--target-kl` (early-stops an update when approx KL exceeds ~1.5× this; fresh runs default 0.03, 0 disables; resumes inherit the run's recorded value, and typing the flag on a resume overrides it), `--seed`, `--root` (default `~/hkrl`), `--headless` (no game window: `-batchmode -nographics`, the mod caps the frame loop at 60 fps; a resume keeps the last session's value, and `--headless`/`--no-headless` override it), `--timescale` (disabled: training always runs at 1× real time; typing a value above 1 is refused, and runs recorded at a higher speed resume at 1×; `replay.py --timescale` still works for the speed-fidelity gate).
