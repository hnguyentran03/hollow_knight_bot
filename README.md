...
From the dashboard's **"Summon a run" panel** you can start, resume, and stop training without a terminal. It spawns `train.py --auto` detached (so the run survives a dashboard exit, and a restarted dashboard picks it back up); `--auto` lets the boot macro drive the game in — a few `reset ... reconnecting` retries in the log tail are normal. The launch form has a boss dropdown, a Headless checkbox, and advanced PPO fields (blank Target KL keeps the 0.03 default); the resume dialog can change a run's instance count and headless/headed on the way back in (untouched fields keep the run's recorded settings). While a run is active the panel's card also shows its recorded config as a facts line — boss, instances, headless, timescale, step target, and target KL. One launched run at a time; Stop is the same graceful path as Ctrl-C. If a launched run dies uncleanly, the panel shows a persistent, dismissible failure card with the reason and a log excerpt instead of silently going quiet — Dismiss or the next launch clears it. Runs started from a terminal are invisible to the panel — stop those in their own terminal. Deleting a run moves it — along with the launcher's log and marker files for that run — into one `~/hkrl/trash/` bundle, never erases anything; exports stay untouched.

The dashboard (`http://127.0.0.1:9700`) also monitors every run: live/stopped status, progress and ETA, elapsed training time, steps/hour, total wins and win rate, learning curves, per-episode reward with each episode's dot colored by win/loss (episodes from before the outcome columns existed stay uncolored) and hoverable for that episode's reward, length, outcome, boss damage, and generation. It is read-only and safe beside a live run. Lower-level health checks:
...
Useful flags: `--boss` (default `hornet1`; ids from `hkrl/bosses.py`), `--instances N`, `--gen-every` (checkpoint interval, default 15000), `--n-steps` / `--batch-size` / `--n-epochs` (fresh runs only), `--target-kl` (early-stops an update when approx KL exceeds ~1.5× this; fresh runs default 0.03, 0 disables; resumes inherit the run's recorded value, and typing the flag on a resume overrides it), `--seed`, `--root` (default `~/hkrl`), `--headless` (no game window: `-batchmode -nographics`, the mod caps the frame loop at 60 fps; a resume keeps the last session's value, and `--headless`/`--no-headless` override it), `--timescale K` (run the game at K× real time, 1-10, default 1; a resume keeps the last session's value, and typing the flag overrides it — `--timescale 1` turns it off; k ≤ 4 is the practical range, higher risks the game outrunning what the machine can simulate).
...
listing its recordings — open one and the matrix renders in place.
...
- If `train.py` fails fast naming an occupied bridge port (9020+), something else holds it — often an old dashboard or a leftover game instance; the error usually names the squatting pid and the exact `kill` command to run before retrying (falling back to how to find it, when the pid can't be resolved). Dashboard launches are refused inline, before anything spawns, on the same check.
